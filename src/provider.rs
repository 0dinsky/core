...
pub(crate) fn legacy_settings_for_addr(addr: &str) -> Result<LegacyProviderSettings> {
    let domain = EmailAddress::new(addr)?.domain.to_lowercase();
    if domain == "nauta.cu" {
        return Ok(LegacyProviderSettings {
    ...
    // error if the number of recipients exceeds the limit.
    // See <https://help.mail.ru/developers/mailing_rules/technical/>.
    if domain == "mail.ru" || domain == "inbox.ru" || domain == "internet.ru" || domain == "bk.ru" || domain == "list.ru" {
        return Ok(LegacyProviderSettings {
            max_smtp_rcpt_to: Some(25),
            ..LegacyProviderSettings::default()
        });
    }
    Ok(LegacyProviderSettings::default())
}
