...
///
/// Uses hybrid algorithms from the **draft** (draft-ietf-openpgp-pqc-12) that
/// ships with `pgp = "0.20.0"` + feature `draft-pqc`. The final standard is
/// RFC 9980 (June 2026). rPGP main has already renamed the feature to `pqc`
/// and updated algorithm IDs / packet formats for the RFC; a future crates.io
/// release will therefore produce keys that are **not** wire-compatible with
/// keys generated by this function. Plan a forced re-key (and preferably a
/// one-time migration helper) when bumping the `pgp` dependency past 0.20.
///
/// Current algorithms (draft-12 numbering):
/// - **Signing** (primary): ML-DSA-65 + Ed25519 composite
/// - **Encryption** (subkey): ML-KEM-768 + X25519 hybrid KEM
///
/// Both are hybrid by design: classical ECC must also be broken for the
/// scheme to fail. Pure lattice schemes are deliberately avoided.
///
/// # Backward compatibility
///
/// Peers that do **not** support PQC fall back to the X25519 component of the
/// hybrid KEM and can still encrypt to us. Decryption works for both
/// hybrid and classic recipients of our messages.
///
/// # Caveats (not bugs of this crate)
///
/// - Mathematical hardness assumptions (Module-LWE etc.) are **not** proven;
///   hybrid construction is mandatory precisely because of historical breaks
///   of other NIST candidates (SIKE, Rainbow).
/// - Side-channel resistance of ML-KEM/ML-DSA software implementations is
///   still an active research area (timing, power, chosen-ciphertext SCA).
///   Do not treat a pure software PQC key as equivalent to a hardened HSM.
/// - This format must **not** be considered production-stable until the
///   `pgp` crate ships an official `pqc` feature matching RFC 9980 and we
///   have performed a controlled key migration.
///
/// # Requirement
///
...
pub(crate) fn create_pqc_keypair(addr: EmailAddress) -> Result<SignedSecretKey> {
    // Composite ML-DSA-65 + Ed25519 for signing (OpenPGP algorithm 30 in draft-12).
...
pub enum EncryptionKind {
    ...
    /// per draft-ietf-openpgp-pqc-12 / future RFC 9980). Treat as experimental
    /// until the `pgp` crate ships the final `pqc` feature and keys are
    /// re-issued under the stable algorithm IDs.
    PostQuantum,
}
...
/// created before `cutoff`, keeping only newer ones.
///
/// OpenPGP itself has **no** native perfect forward secrecy. The best
/// approximation available is periodic encryption-subkey rotation followed
/// by guaranteed deletion of the old secret material (this function).
/// Autocrypt v2 / "pretty good forward secrecy" follows the same pattern.
///
/// [`rotate_encryption_subkey`] alone does **not** provide forward secrecy: it
/// keeps every previous encryption subkey around indefinitely so that
/// messages already in flight can still be decrypted. This function is the
/// actual "forget" step. Call it only once you're confident that any message
/// encrypted to the subkeys being pruned has either been delivered and
/// decrypted, or can be considered permanently lost — i.e. `cutoff` should be
/// well before "now", with a generous grace period on top of your rotation
/// interval to account for offline recipients and slow delivery.
///
/// Once pruned, the discarded subkey's secret material is gone from the
/// returned key: there is no way to recover it, and messages that were only
/// ever encrypted to that subkey become permanently unreadable. This is
/// intentional — it is the whole point of forward secrecy. Do **not** add a
/// "recovery by re-encryption" path; that would re-introduce the very
/// material this function erases and defeat the purpose.
///
/// Fails without modifying anything if pruning would remove every remaining
/// encryption subkey, since a key must always retain at least one usable
/// encryption subkey.
pub fn forget_expired_encryption_subkeys(
    ...
    let secret_subkeys: Vec<_> = existing
        .secret_subkeys
        .iter()
